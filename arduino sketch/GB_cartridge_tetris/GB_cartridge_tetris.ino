...
void loop() {
  ...
  if (digitalRead(start_switch) == LOW) //b0 purple
  {
  ...
  if (digitalRead(analyze_switch) == LOW) //b4yellow
  {
    ...
    {
      byte var = pgm_read_byte_near(tetris_1 + i);
      GB_write_byte_audio(i, var);
    ...
    for (int i=0; i<4096; i++)
    {
      byte var = pgm_read_byte_near(tetris_2 + i);
      GB_write_byte_audio(i, var);
      uint8_t in = GB_read_byte(i);
      if (var != in)
      {
    ...
    for (int i=0; i<4096; i++)
    {
      byte var = pgm_read_byte_near(tetris_3 + i);
      GB_write_byte_audio(i, var);
      uint8_t in = GB_read_byte(i);
      if (var != in)
      {
    ...
    for (int i=0; i<4096; i++)
    {
      byte var = pgm_read_byte_near(tetris_4 + i);
      GB_write_byte_audio(i, var);
      uint8_t in = GB_read_byte(i);
      if (var != in)
      {
    ...
    for (int i=0; i<4096; i++)
    {
      byte var = pgm_read_byte_near(tetris_5 + i);
      GB_write_byte_audio(i, var);
      uint8_t in = GB_read_byte(i);
      if (var != in)
      {
    ...
    for (int i=0; i<4096; i++)
    {
      byte var = pgm_read_byte_near(tetris_6 + i);
      GB_write_byte_audio(i, var);
      uint8_t in = GB_read_byte(i);
      if (var != in)
      {
    ...
    for (int i=0; i<4096; i++)
    {
      byte var = pgm_read_byte_near(tetris_7 + i);
      GB_write_byte_audio(i, var);
      uint8_t in = GB_read_byte(i);
      if (var != in)
      {
    ...
    for (int i=0; i<4096; i++)
    {
      byte var = pgm_read_byte_near(tetris_8 + i);
      GB_write_byte_audio(i, var);
      uint8_t in = GB_read_byte(i);
      if (var != in)
      {
...
//    
//    inByte = Serial.read();
//    analogWrite(led_red, inByte);
//    analogWrite(led_green, inByte);
//    digitalWrite(led_blue, HIGH);
//    
//    GB_write_byte_audio(input_addr, inByte);
//    input_addr++;
//  }
//  else
//  {
